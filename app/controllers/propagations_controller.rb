class PropagationsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @propagation.update(propagation_params)
        ...
        format.json { render :show, status: :ok, location: @propagation }
      else
  ...
  def destroy
    @propagation.destroy
    respond_to do |format|
      format.html { redirect_to propagations_url, notice: 'Propagation was successfully destroyed.' }
  ...
  private
    ...
    def propagation_params
      params.require(:propagation).permit(:name, :explain)
    end
end
